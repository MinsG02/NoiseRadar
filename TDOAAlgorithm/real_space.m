...
chirp_signal_base = [chirp_part; zeros(padding_len, 1)];

% **(수정 1) 신호 100배 증폭 (High SNR)**
chirp_signal = chirp_signal_base * 100; 
...
disp('시뮬레이션 시작... (fs=192kHz, High SNR)');
...
%% --- 결과 출력 ---
disp('--- 4-Mic + 192kHz + 10cm Grid (High SNR) ---');
...
for k = 1:size(tx_pos,1)
    ...
    plot(t_total, chirp_signal,'b','DisplayName','Original Signal (x100)'); hold on;
...
title('TDOA 위치 추정 (4-Mic, 192kHz, 10cm Grid, High SNR)'); 
hold off;